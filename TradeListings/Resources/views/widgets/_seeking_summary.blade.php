    ...
    <div class="col-sm-2">
        ...
        <div class="col-md">
            @foreach($listing->data['seeking_wishlists'] as $wishlist)
                <a href="{{ url('user/'.$user->name.'/wishlists/'.$wishlist) }}">{{ $wishlist == 'default' ? 'Default' : App\Models\User\Wishlist::where('user_id', $listing->user->id)->where('id', $wishlist)->first()->name }}</a>{{ !$loop->last ? ', ' : '' }}
            @endforeach
        </div>
    </div>
@endif

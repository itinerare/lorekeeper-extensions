        ...
        <div class="card-body user-items">
            <table class="table table-sm">
                ...
                <tbody>
                    ...
                    <tr class="d-flex">
                        <td class="col-3">
                            @if(isset($items[$itemRow['asset']->item_id]->image_url)) <img class="small-icon" src="{{ $items[$itemRow['asset']->item_id]->image_url }}"> @endif <a href="/world/items?name={{ $items[$itemRow['asset']->item_id]->name }}">{!! $items[$itemRow['asset']->item_id]->name !!}</a>
                            @isset($wishlists)
                                @include('wishlists::_wishlist_check', ['item' => $items[$itemRow['asset']->item_id]])
                            @endisset
                        </td>
                        <td class="col-4">
                            {!! array_key_exists('data', $itemRow['asset']->data) ? ($itemRow['asset']->data['data'] ? $itemRow['asset']->data['data'] : 'N/A') : 'N/A' !!}
                        </td>
                        <td class="col-3">
                            {!! array_key_exists('notes', $itemRow['asset']->data) ? ($itemRow['asset']->data['notes'] ? $itemRow['asset']->data['notes'] : 'N/A') : 'N/A' !!}
                        </td>
                        <td class="col-2">
                            {!! $itemRow['quantity'] !!}
                        </td>

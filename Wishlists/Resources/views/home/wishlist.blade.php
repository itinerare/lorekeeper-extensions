...
@section('home-title')
    Wishlists: {{ $wishlist ? $wishlist->name : 'Default' }}
@endsection
                ...
                <div class="d-flex row flex-wrap col-12 mt-1 pt-2 px-0 ubt-top">
                    <div class="col-5 col-md-4">
                        @if (isset($item->item->image_url))
                            <img class="small-icon" src="{{ $item->item->image_url }}" alt="{{ $item->item->name }}">
                        @endif{!! $item->item->displayName !!}
                    </div>
                            ...
                            <div class="dropdown-menu" aria-labelledby="move-{{ $item->item->id }}">
                                @if ($wishlist)
                                    ...
                                    <a class="dropdown-item" href="#" onclick="document.getElementById('wishlistForm-0-{{ $item->item->id }}').submit();">
                                        Default
                                        @if ((new Extensions\Wishlists\Models\Wishlist())->itemCount($item->id, $user))
                                            - {{ (new Extensions\Wishlists\Models\Wishlist())->itemCount($item->item->id, $user) }} In Wishlist
                                        @endif
                                    </a>
                                    {!! Form::close() !!}
                                @endif
                                @foreach ($user->wishlists as $targetWishlist)
                                    @if (!$wishlist || $targetWishlist->id != $wishlist->id)
                                        ...
                                        <a class="dropdown-item" href="#" onclick="document.getElementById('wishlistForm-{{ $targetWishlist->id }}-{{ $item->item->id }}').submit();">
                                            {{ $targetWishlist->name }}
                                            @if ($targetWishlist->itemCount($item->item->id, $user))
                                                - {{ $targetWishlist->itemCount($item->item->id, $user) }} In Wishlist
                                            @endif
                                        </a>
                                        {!! Form::close() !!}
                                    @endif

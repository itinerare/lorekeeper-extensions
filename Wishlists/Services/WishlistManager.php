...
namespace Extensions\Wishlists\Services;
    ...
    */
    ...
    public function createWishlist($data, $user) {
        ...
        try {
            ...
            if (Wishlist::where('user_id', $user->id)->where('name', $data['name'])->exists()) {
                throw new \Exception('You have already created a wishlist with this name.');
            }
    ...
    public function updateWishlist($data, $wishlist, $user) {
        ...
        try {
            ...
            if ($wishlist->user_id != $user->id) {
                throw new \Exception('This wishlist does not belong to you.');
            }
            // Check that the user does not already have a wishlist with this name
            if (Wishlist::where('user_id', $user->id)->where('name', $data['name'])->exists()) {
                throw new \Exception('You have already created a wishlist with this name.');
    ...
    public function updateWishlistItem($wishlist, $item, $data, $user) {
        ...
        try {
            ...
            if (isset($data['count']) && $data['count'] == 0) {
                ...
                if ($wishlistItem->count == 9999) {
                    throw new \Exception('Cannot add wishlist item as count would exceed 9999.');
                }

                // Update wishlist item
                $wishlistItem->update([
                    'count' => $data['count'] ?? $wishlistItem->count += 1,
                ]);
            }
    ...
    public function moveWishlistItem($wishlist, $item, $data, $user) {
        ...
        try {
            ...
            if ($data['source_id'] != 0) {
                ...
                if (!$source) {
                    throw new \Exception('Invalid origin wishlist.');
                }
                if ($source->user_id != $user->id) {
                    throw new \Exception('The origin wishlist does not belong to you.');
                }
            }
            ...
            if ($wishlistItem) {
                // Check that the maximum would not be exceeded
                if (($wishlistItem->count + $sourceItem->count) > 9999) {
                    throw new \Exception('Cannot move wishlist item as count would exceed 9999.');
                }
